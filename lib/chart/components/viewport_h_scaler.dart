...
class GPointViewPortAutoScaleStrategyLatest implements GPointViewPortAutoScaleStrategy {
  ...
  GRange getScale({
    ...
    double lastPoint =
        ((dataSource.isEmpty && pointViewPort.isValid) ? pointViewPort.endPoint : dataSource.lastPoint.toDouble()) + endSpacingPoints;
    int viewPointCount = (panel.graphArea().width / pointViewPort.defaultPointWidth).ceil();
    return GRange.range((lastPoint - viewPointCount).toDouble(), lastPoint.toDouble());
  }
}
...
class GPointViewPortAutoScaleStrategyAlignTo implements GPointViewPortAutoScaleStrategy {
  ...
  const GPointViewPortAutoScaleStrategyAlignTo({this.alignToPosition})
      : assert((alignToPosition == null) || (alignToPosition >= 0 && alignToPosition <= 1));
  @override
  GRange getScale({
    required GChart chart,
    required GPanel panel,
    ...
    double position =
        alignToPosition ?? (chart.crosshair.crossPosition.begin! - panel.graphArea().left) / panel.graphArea().width;
    double alignToPoint = pointViewPort.startPoint + (pointViewPort.endPoint - pointViewPort.startPoint) * position;
    return GRange.range(alignToPoint - viewPointCount * position, alignToPoint + viewPointCount * (1 - position));
  }
}

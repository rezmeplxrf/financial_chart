...
class GPanelRender extends GRender<GPanel, GPanelTheme> {
  ...
  void doRenderAxes({required Canvas canvas, required GChart chart, required GPanel panel, bool inside = false}) {
    for (int n = 0; n < panel.valueAxes.length; n++) {
      final axis = panel.valueAxes[n];
      if (!axis.visible) {
        continue;
      }
      if ((inside && !axis.position.isInside) || (!inside && axis.position.isInside)) {
        continue;
      }
  ...
  void doRenderGraphs({required Canvas canvas, required GChart chart, required GPanel panel}) {
  ...
  void doRenderTooltips({required Canvas canvas, required GChart chart, required GPanel panel}) {

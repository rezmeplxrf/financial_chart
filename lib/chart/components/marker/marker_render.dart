...
class GAxisMarkerRender extends GMarkerRender<GAxisMarker, GAxisMarkerTheme> {
  ...
  void renderValueAxisRangeMarkers({
    ...
    for (final range in valueRanges) {
      ...
      addRectPath(
        ...
        rect: Rect.fromLTRB(axisArea.left, max(top, axisArea.top), axisArea.right, min(bottom, axisArea.bottom)),
      );
    }
    drawPath(canvas: canvas, path: valueRangePath, style: theme.valueRangeStyle!);
  }
  ...
  void renderPointAxisRangeMarkers({
    ...
    for (final range in pointRanges) {
      final start = pointViewPort.pointToPosition(axisArea, range.begin!.toDouble());
      final end = pointViewPort.pointToPosition(axisArea, range.end!.toDouble());
      addRectPath(
        toPath: pointRangePath,
        rect: Rect.fromLTRB(max(start, axisArea.left), axisArea.top, min(end, axisArea.right), axisArea.bottom),
      );
    }
    drawPath(canvas: canvas, path: pointRangePath, style: theme.pointRangeStyle!);
  }
  ...
  void renderPointAxisLabelMarkers({
    ...
    for (final point in points) {
      ...
      final text = (axis.pointFormatter ?? chart.dataSource.pointValueFormater).call(point, pointValue);
      drawPointAxisLabel(
        canvas: canvas,
        text: text,
        axis: axis,
...
abstract class GGraphMarkerRender<M extends GGraphMarker, T extends GGraphMarkerTheme> extends GMarkerRender<M, T> {

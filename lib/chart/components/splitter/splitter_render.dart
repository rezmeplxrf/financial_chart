...
class GSplitterRender extends GRender<GSplitter, GSplitterTheme> {
  ...
  void doRender({
    ...
    final crossPosition = chart.crosshair.getCrossPosition() ?? const Offset(double.infinity, double.infinity);
    for (int p = 0; p < chart.panels.length - 1; p++) {
      final panel = chart.panels[p];
      if (panel.splitterArea().contains(crossPosition) || p == chart.controller.resizingPanelIndex) {
  ...
  void doRenderSplitter({
    ...
    Path linePath = Path();
    addLinePath(toPath: linePath, x1: area.left, y1: center.dy, x2: area.right, y2: center.dy);
    drawPath(canvas: canvas, path: linePath, style: theme.lineStyle);

    Path rectPath = Path();
    addRectPath(
        toPath: rectPath,
        rect: Rect.fromCenter(center: center, width: theme.handleWidth, height: area.height),
    ...
    addLinePath(
        ...
        x2: center.dx - handleLineWidthHalf,
        y2: center.dy + handleLineOffset);
    drawPath(canvas: canvas, path: handleLinesPath, style: theme.handleLineStyle);
  }
}

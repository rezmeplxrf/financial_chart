...
class GGraphGridsRender extends GGraphRender<GGraphGrids, GGraphGridsTheme> {
  ...
  void doRenderGraph({
    ...
    graph.pointTickerStrategy.pointTicks(viewSize: area.width, viewPort: pointViewPort).forEach((point) {
      double dx = pointViewPort.pointToPosition(area, point.toDouble());
      addLinePath(toPath: tickLinesPath, x1: dx, y1: area.top, x2: dx, y2: area.bottom);
    });
    graph.valueTickerStrategy.valueTicks(viewSize: area.height, viewPort: valueViewPort).forEach((value) {
      double dy = valueViewPort.valueToPosition(area, value);
      addLinePath(toPath: tickLinesPath, x1: area.left, y1: dy, x2: area.right, y2: dy);
    });

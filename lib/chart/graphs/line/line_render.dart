...
class GGraphLineRender extends GGraphRender<GGraphLine, GGraphLineTheme> {
  ...
  void doRenderGraph({
    ...
    for (int i = 0; i < points.length - 1; i++) {
      addLinePath(toPath: tickLinesPath, x1: points[i].x, y1: points[i].y, x2: points[i + 1].x, y2: points[i + 1].y);
    }
  ...
  bool hitTest({
    ...
    return super.hitTestLines(lines: [_hitTestLinePoints], position: position, epsilon: epsilon);
  }
}

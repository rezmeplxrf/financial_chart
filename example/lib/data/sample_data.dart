...
Future<GDataSource> loadSampleData({int simulateLatencyMillis = 0, bool simulateEmpty = false}) async {
  ...
  final indicatorInput = IndicatorInputImpl(response.candlesData.map((e) {
    return IndicatorOHLCImpl(e.open, e.close, e.high, e.low);
  }).toList());

  final smaIndicator = SMAIndicator<IndicatorResultImpl>(CloseValueIndicator(indicatorInput), 14);
  final emaIndicator = EMAIndicator<IndicatorResultImpl>(CloseValueIndicator(indicatorInput), 14);
  final macdIndicator = MACDIndicator<IndicatorResultImpl>(indicatorInput);
  final macdHistogramIndicator = MACDHistogramIndicator<IndicatorResultImpl>.fromIndicator(
    macdIndicator,
    SignalMACDIndicator<IndicatorResultImpl>.fromIndicator(MACDIndicator<IndicatorResultImpl>(indicatorInput)),
  );
  final ichimokuBase = IchimokuBaseLineIndicator<IndicatorResultImpl>(indicatorInput);
  final ichimokuConversion = IchimokuConversionLineIndicator<IndicatorResultImpl>(indicatorInput);
  final ichimokuSpanA = IchimokuSpanAIndicator<IndicatorResultImpl>(indicatorInput);
  final ichimokuSpanB = IchimokuSpanBIndicator<IndicatorResultImpl>(indicatorInput);
  final ichimokuLagging = IchimokuLaggingSpanIndicator<IndicatorResultImpl>(indicatorInput);
  ...
  final bbl =
      BollingerBandsLowerIndicator(smaIndicator, StandardDeviationIndicator(CloseValueIndicator(indicatorInput), 14));
  final bbu =
      BollingerBandsUpperIndicator(smaIndicator, StandardDeviationIndicator(CloseValueIndicator(indicatorInput), 14));
  final rsi = RSIIndicator.fromIndicator(CloseValueIndicator(indicatorInput), 14);
  ...
  const seriesProperties = [
    ...
    GDataSeriesProperty(key: keyClose, label: 'Close', precision: pricePrecision),
    ...
    GDataSeriesProperty(key: keyMACDHist, label: 'MACD Histogram', precision: pricePrecision),
    GDataSeriesProperty(key: keyIchimokuBase, label: 'Ichimoku Base', precision: pricePrecision),
    GDataSeriesProperty(key: keyIchimokuConversion, label: 'Ichimoku Conversion', precision: pricePrecision),
    GDataSeriesProperty(key: keyIchimokuSpanA, label: 'Ichimoku Span A', precision: pricePrecision),
    GDataSeriesProperty(key: keyIchimokuSpanB, label: 'Ichimoku Span B', precision: pricePrecision),
    GDataSeriesProperty(key: keyIchimokuLagging, label: 'Ichimoku Lagging', precision: pricePrecision),
    GDataSeriesProperty(key: keyFastStoch, label: 'Fast Stochastic', precision: 2),
    GDataSeriesProperty(key: keySmoothedFastStoch, label: 'Smoothed Fast Stochastic', precision: 2),
    GDataSeriesProperty(key: keySlowStoch, label: 'Slow Stochastic', precision: 2),
    GDataSeriesProperty(key: keySmoothedSlowStoch, label: 'Smoothed Slow Stochastic', precision: 2),
    GDataSeriesProperty(key: keyBBL, label: 'Bollinger Bands Lower', precision: pricePrecision),
    GDataSeriesProperty(key: keyBBU, label: 'Bollinger Bands Upper', precision: pricePrecision),
    GDataSeriesProperty(key: keyRSI, label: 'RSI', precision: 2),
    GDataSeriesProperty(key: keyADX, label: 'ADX', precision: 2),
  ];

  int index = 0;
  final dataList = response.candlesData.map((candle) {
    index++;
    return GData<int>(
  ...
  final dataSource = GDataSource<int, GData<int>>(
      ...
      priorDataLoader: ({required int toPointExclusive, required int toPointValueExclusive, required int pointCount}) async {
        if (kDebugMode) {
          print('priorDataLoader: $toPointValueExclusive, $pointCount');
        }
        return Future.delayed(latencyDuration, () {
          final index = dataList.indexWhere((element) => element.pointValue >= toPointValueExclusive);
          if (index <= 0) {
            return [];
          }
      ...
      afterDataLoader: ({required int fromPointExclusive, required int fromPointValueExclusive, required int pointCount}) async {
        if (kDebugMode) {
          print('afterDataLoader: $fromPointValueExclusive, $pointCount');
        }
        return Future.delayed(latencyDuration, () {
          final index = dataList.indexWhere((element) => element.pointValue > fromPointValueExclusive);
          if (index < 0) {
            return [];
          }
          return dataList.sublist(index, min(index + pointCount, dataList.length));
        });
      });
  return dataSource;
}
...
class IndicatorInputImpl implements IndicatorDataInput {
  ...
  @override
  IndicatorResult createResult(int index, double value) => IndicatorResultImpl(value);
}
